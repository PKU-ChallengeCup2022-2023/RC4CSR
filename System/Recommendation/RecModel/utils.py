# Reference: https://github.com/zombak79/vasp
...
class Data:
    ...
    def __init__(self, d=str(), pruning=False):
        self.directory = '' # D:\some_projects\Cup\\rec_algorithm\\second_version\\
        if pruning:
    ...
    def create_splits(
        self, n, k_test, shuffle=True, n_fold=True, generators=True, batch_size=1024
    ):
        """
    ...
    def load_splits(self, split=0):
        if split == 0:
            for i in range(len(self.splits)):
                d = "split_" + str(i + 1)
                print(d)
                self.splits[i].train_users = (
                    pd.read_json(d + "/train_users.json").userid.apply(str).to_frame()
                )
                self.splits[i].test_users = (
                    pd.read_json(d + "/test_users.json").userid.apply(str).to_frame()
                )
                self.splits[i].generators()
        else:
            d = "split_" + str(split)
            print(d)
            self.splits[0].train_users = (
                pd.read_json(d + "/train_users.json").userid.apply(str).to_frame()
            )
            self.splits[0].test_users = (
                pd.read_json(d + "/test_users.json").userid.apply(str).to_frame()
            )
    ...
    def read_users(self, p=""):
        ...
        print(self.directory + "users" + p + ".json")
    ...
    def read_purchases_txt(self, p=""):
        ...
        self.purchases_txt = pd.read_json(
            self.directory + "purchases_txt" + p + ".json"
        )
...
class Split:
    ...
    def __init__(
        self, data, k_test, shuffle=True, index=0, generator=True, batch_size=1024
    ):
        """
        ...
        self.train_users = self.all_users[
            ~self.all_users.userid.isin(self.test_users.userid)
        ]
        self.validation_users = self.train_users.iloc[index : index + k_test]
        self.train_users = self.train_users[
            ~self.train_users.userid.isin(self.validation_users.userid)
        ]
        if generator:
    ...
    def train_purchases_txt(self):
        return self.master_data.purchases_txt[
            self.master_data.purchases_txt.userid.isin(self.train_users.userid)
        ].copy(deep=True)

    def test_purchases_txt(self):
        return self.master_data.purchases_txt[
            self.master_data.purchases_txt.userid.isin(self.test_users.userid)
        ].copy(deep=True)

    def validation_purchases_txt(self):
        return self.master_data.purchases_txt[
            self.master_data.purchases_txt.userid.isin(self.validation_users.userid)
        ].copy(deep=True)
...
class SplitGenerator(tf.keras.utils.Sequence):
    ...
    def get_basket_np(self, items):
        if self.n_ratings:
            return np.vstack(
                [self.embeddings_dict.get(x, self.null_val) for x in items.split(",")]
            )

        return np.vstack(
            [self.embeddings_dict.get(x, self.null_val) for x in set(items.split(","))]
        )
    ...
    def __getitem__(self, index):
        ...
        data_slice = self.data_np[
            self.batch_size * index : self.batch_size * index + self.batch_size
        ]
        ...
        if self.p50_splits:
            data_slice2 = self.data_np[
                self.batch_size * index2 : self.batch_size * index2 + self.batch_size
            ]
            data_slice3 = self.data_np[
                self.batch_size * index3 : self.batch_size * index3 + self.batch_size
            ]

        if (
            self.p2575_splits
            or self.p7525_splits
            or self.p2525_splits
            or self.p7575_splits
        ):
            data_slice4 = self.data_np[
                self.batch_size * index4 : self.batch_size * index4 + self.batch_size
            ]
            data_slice5 = self.data_np[
                self.batch_size * index5 : self.batch_size * index5 + self.batch_size
            ]
...
class Evaluator:
    ...
    def __init__(
        self, split, method="leave_random_20_pct_out", data="test", debug=False
    ):
        assert method in [
            "leave_random_20_pct_out",
            "1_20",
            "2_20",
            "3_20",
            "4_20",
        ...
        if data == "val":
            ...
            self.ivx = (
                split.validation_gen.data.set_index(split.validation_gen.data.userid)
                .sort_index()
                .itemids.apply(lambda x: x.split(","))
            )
        else:
            print("Creating test split evaluator with", method, "method.")
            self.ivx = (
                split.test_gen.data.set_index(split.test_gen.data.userid)
                .sort_index()
                .itemids.apply(lambda x: x.split(","))
            )
        if debug:
    ...
    def update(self, m, chunk=1000):
        ...
        self.pr = np.vstack(
            [
                m.predict(self.iv[chunk * x : chunk * (x + 1)])
                for x in range(len(self.iv) // chunk)
            ]
        )
    ...
    def get_ncdg(self, k):
        ...
        z = zip(
            [[self.split.master_data.toki.index_word[b] for b in a] for a in idx_topk],
            self.tpx_set,
        )
        n = np.array(
            [
                (np.array([1 if x in true else 0 for x in pred]) * tp).sum()
                for pred, true in z
            ]
        )
        d = np.array(
            [(np.ones(min(k, len(x))) * tp[: len(x)]).sum() for x in self.tpx_set]
        )
        return (n / d).mean()
    ...
    def get_hr(self, k):
        ...
        z = zip(
            [
                set([self.split.master_data.toki.index_word[s] for s in a[:k]])
                for a in idx
            ],
            self.tpx_set,
        )
...
class Model:
    ...
    def __init__(self, split, name):
        ...
        self.metrics = {
            "Recall@5": {
                "k": 5,
                "method": self.split.evaluator.get_recall,
                "value": None,
            },
            "Recall@20": {
                "k": 20,
            ...
            "NCDG@100": {
                "k": 100,
                "method": self.split.evaluator.get_ncdg,
                "value": None,
            },
            "Coverage@5": {
                "k": 5,
                "method": self.split.evaluator.get_coverage,
                "value": None,
            },
            "Coverage@20": {
                "k": 20,
    ...
    def test_model(self):
        ...
        e.update(self.model)
        print(
            "Results for test set: Recall@20=",
            e.get_recall(20),
            ", Recall@50=",
            e.get_recall(50),
            ", NCDG@100=",
    ...
    def test_model_val(self):
        ...
        e.update(self.model)
        print(
            "Results for validation set: Recall@20=",
            e.get_recall(20),
            ", Recall@50=",
            e.get_recall(50),
            ", NCDG@100=",
...
class MetricsCallback(tf.keras.callbacks.Callback):
    ...
    def __init__(self, rsmodel):
        ...
        self.best_ncdg100 = 0.0
        self.best_ncdg100_epoch = 0
        self.best_recall20 = 0.0
        self.best_recall20_epoch = 0
        self.best_recall50 = 0.0
    ...
    def recall20_watch(self):
        if self.eval_metrics[self.epoch]["Recall@20"] > self.best_recall20:
            ...
            self.model.save_weights(
                self.rsmodel.name + "_best_recall_20/" + self.rsmodel.name
            )
            self.best_recall20 = self.eval_metrics[self.epoch]["Recall@20"]
            self.best_recall20_epoch = self.epoch
            with open(
                self.rsmodel.name + "_best_recall_20/" + "epoch.txt", "w"
            ) as text_file:
    ...
    def recall50_watch(self):
        if self.eval_metrics[self.epoch]["Recall@50"] > self.best_recall50:
            ...
            self.model.save_weights(
                self.rsmodel.name + "_best_recall_50/" + self.rsmodel.name
            )
            self.best_recall50 = self.eval_metrics[self.epoch]["Recall@50"]
            self.best_recall50_epoch = self.epoch
            with open(
                self.rsmodel.name + "_best_recall_50/" + "epoch.txt", "w"
            ) as text_file:
    ...
    def ncdg_100_watch(self):
        if self.eval_metrics[self.epoch]["NCDG@100"] > self.best_ncdg100:
            ...
            self.model.save_weights(
                self.rsmodel.name + "_best_ncdg_100/" + self.rsmodel.name
            )
            self.best_ncdg100 = self.eval_metrics[self.epoch]["NCDG@100"]
            self.best_ncdg100_epoch = self.epoch
            with open(
                self.rsmodel.name + "_best_ncdg_100/" + "epoch.txt", "w"
            ) as text_file:
    ...
    def get_history_df(self):
        ...
        outt1 = {"epochs": [x for x in self.rsmodel.mc.loss_metrics.keys()]}

        outt2 = {"epochs": [x for x in self.rsmodel.mc.eval_metrics.keys()]}
    ...
    def plot_history(self):
        return (
            self.get_history_df()
            .set_index(self.history_df.epochs, drop=True)
            .iloc[:, 1:]
            .plot(figsize=(20, 10))
        )
    ...
    def calc_tsne(self):
        ...
        np.fill_diagonal(input_single_item_matrix, 1.0)
        qqq = scale_d(self.model.predict(input_single_item_matrix)).numpy() * 0.99
        np.fill_diagonal(qqq, 1.0)
